...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        await databaseManager.getKnex()('users')
            .where({ id: user.id })
...
router.get('/profile/:userId', async (req, res) => {
    try {
        const user = await databaseManager.getKnex()('users').where({ id: req.params.userId }).first();
...
router.put('updatePassword/:userId', async (req, res) => {
    ...
    try {
        ...
        await databaseManager.getKnex()('users').where({ id: req.params.userId }).update({ password: hashedPassword });
...
router.get('/getCouriers', userAuthMiddleware, async (req, res) => {
    try {
        const users = await databaseManager.getKnex()('users')
...
router.get('/getAllCouriersByAdminId', userAuthMiddleware, async (req, res) => {
    try {
        ...
        const userRights = await databaseManager.getKnex()('user_rights').where({ user_id: req.user.id })
...
router.get('/searchCourier', userAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        const couriers = await databaseManager.getKnex()('users')
            .join('user_rights', 'users.id', 'user_rights.user_id')
            .join('rights', 'user_rights.right_id', 'rights.id')
            .where('rights.right_code', 2)
            .where(function () {
            ...
            .whereNotIn('users.id', databaseManager.getKnex()('user_routes').select('courier_id'))
            .select('users.*');
...
router.post('/addCourier', userAuthMiddleware, async (req, res) => {
    try {
        ...
        const phoneExists = await databaseManager.getKnex()('users').where('phone', userData.phone).first();
        if (phoneExists) {
        ...
        let newUserId;
        // let rightCode;
        const userEmail = await databaseManager.getKnex()('users').where('email', email).first();
        if (!userEmail) {
            // Insert the new user into the database
            [newUserId] = await databaseManager.getKnex()('users')
                .insert(userData)
                .returning('id');

            const rightCode = await databaseManager.getKnex()('rights').where('right_code', 2).first();

            await databaseManager.getKnex()('user_rights')
...
router.delete('/deleteCourier/:courierId', userAuthMiddleware, async (req, res) => {
    try {
        ...
        const user = await databaseManager.getKnex()('users').where({ id: courierId }).first();
        if (!user) return sendJsonResponse(res, false, 404, "Curierul nu există!", []);

        console.log('user', user);

        await databaseManager.getKnex()('users').where({ id: courierId }).del();
