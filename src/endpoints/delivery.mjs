...
router.post('/addOrdersToDelivery/:deliveryId', userAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        await db('orders').whereIn('id', order_ids).update({ delivery_id: deliveryId });
        const orders = await db('orders').whereIn('id', order_ids);
...
router.post('/assignCourierToDelivery/:deliveryId', async (req, res) => {
    ...
    try {
        ...
        await db('delivery').where({ id: deliveryId }).update({ courier_id: courier_id });
        const updated = await db('delivery').where({ id: deliveryId }).first();
